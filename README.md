...
**The six agents and the plans that unlock them**
...
| Customer Agent | `CHURN_RISK`, `REPEAT_PURCHASE` | Start ($79/mo) |
| Pricing Agent | `PRICING_UPLIFT` | Growth ($199/mo) |
| Product Agent | `CROSS_SELL` | Commander ($399/mo) |
| Executive Agent | `WEEKLY_HEALTH_DIGEST` | Commander ($399/mo) |

Trial unlocks 2 agents, Start 3, Growth 4, Commander all 6 — enforced
...
- **Plan metering** — `ai_recommendations_month` (Trial 10 / Start 150 / Growth 300 / Commander unlimited) is enforced server-side via `billing_usage`, with a usage ring, near-limit warning, and hard-block upgrade CTA in the UI. Plan limits live in one place: `packages/types/src/plans.ts` (`PLAN_ENTITLEMENT_LIMITS`).
