...
const main = (): void => {
  logger.info('ProfitPilot Worker starting', {
    entry: 'apps/worker/dist/main.js',
    port: Number(process.env.WORKER_PORT ?? '3100'),
    queue: process.env.UPSTASH_REDIS_REST_URL?.trim() && process.env.UPSTASH_REDIS_REST_TOKEN?.trim() ? 'upstash' : 'in-memory',
...
try {
  ...
  logger.error('ProfitPilot Worker failed to start', { error: error instanceof Error ? error.message : String(error) })
  process.exit(1)
}
