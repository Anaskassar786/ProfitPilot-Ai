...
describe('billing worker ticks', () => {
  ...
  it('returns expiring trials for the hourly nudge tick', async () => { const ledger = new TrialAndGiftLedger(); ledger.startTrial('s', 0, 1); await expect(runHourlyTrialNudge(ledger, 0)).resolves.toHaveLength(1) })
})
