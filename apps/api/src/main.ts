...
const bootstrap = createF5Bootstrap(process.env)
const app = bootstrap === null
  ? createApi({ logger, readinessChecks: readinessChecksFromEnv(process.env) })
  : createApi({ logger, readinessChecks: readinessChecksFromEnv(process.env), shopify: bootstrap.shopify, dataPlane: bootstrap.dataPlane, ai: bootstrap.ai, billing: bootstrap.billing, admin: bootstrap.admin })
