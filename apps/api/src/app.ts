...
export function createApi(dependencies: ApiDependencies): Express {
  ...
  // Data Exports owns /exports/* (overview, history, per-dataset generation).
  // Mounted before the automation router so the merchant page is served by the
  // plan-gated service while the legacy POST /exports writer stays available.
  if (dependencies.exports) app.use(createExportsRouter(dependencies.exports))
