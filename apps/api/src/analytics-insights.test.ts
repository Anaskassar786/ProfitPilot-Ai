...
describe('analytics deterministic insights', () => {
  ...
  it('supports a Commander-grade 30-day predictive horizon', () => { const result = linearForecast(revenue, 30); expect(result.status).toBe('available'); expect(result.points).toHaveLength(30); expect(result.message).toContain('30-day') })
  ...
  it('enforces plan gating before returning anomaly and premium features', async () => { const analytics = { read: async () => snapshot, readCatalog: async () => catalog }; const orders = { list: async () => [] }; const trial = new AnalyticsInsightsService(analytics, { get: async () => null }, orders, new InMemoryAnalyticsQueryUsage(), null, () => 1); const result = await trial.get(tenant); expect(result.anomalies).toBeNull(); expect(result.channels).toEqual([]); expect(result.advancedForecast).toBeNull(); expect(result.locked).toContainEqual({ feature: 'anomaly_detection', requiredPlan: 'start' }); expect(result.locked).toContainEqual({ feature: 'cohort_analysis', requiredPlan: 'growth' }); expect(result.locked).toContainEqual({ feature: 'custom_ai_queries', requiredPlan: 'commander' }); await expect(trial.geography(tenant)).rejects.toMatchObject({ status: 403 }); await expect(trial.comparisons(tenant)).rejects.toMatchObject({ status: 403 }) })
})
