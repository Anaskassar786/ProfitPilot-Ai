...
class PdfDocument {
  ...
  public serialize(): Buffer {
    ...
    objects.push('<< /Type /Info /Title (GrowthIQ Board Report) /Producer (ProfitPilot GrowthIQ) >>')
    // Fonts: objects 4-9.
...
export function renderExecutiveReportPdf(input: ExecutivePdfInput): Buffer {
  ...
  const brand = white.brandName?.trim() || 'ProfitPilot GrowthIQ'
  const footer = white.footerText?.trim() || 'Generated by ProfitPilot GrowthIQ from real store data'
