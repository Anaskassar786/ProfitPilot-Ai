...
export function createAutomationRouter(dependencies: AutomationRouteDependencies): Router {
  ...
  router.post('/automation/runs/:runId/cancel', asyncRoute(async (request) => {
    const tenant=storeFromRequest(request);await permit(dependencies,request,tenant,'automation:write');if(!dependencies.runs)throw unavailable('Run cancellation');return dependencies.runs.transition(tenant,param(request.params.runId),'CANCELLED',{error:'Cancelled by merchant'})
  }))
  ...
  router.post('/automation/templates/:templateId/install', asyncRoute(async (request) => {
    const body=record(request.body);const tenant=requiredString(body.storeId,'storeId');await permit(dependencies,request,tenant,'automation:write');await assertWorkflowAccess(dependencies,tenant,await dependencies.workflows.count(tenant));const template=required(templateFor(param(request.params.templateId)),'Template not found');const plan=await planFor(dependencies,tenant);if(!planAllowsTemplate(plan,template.minimumPlan))throw new AppError('PAYMENT_REQUIRED','Upgrade Plan to install this template',402,{reason:'UPGRADE_REQUIRED'});const name=requiredString(body.name,'name');const definition=installTemplate(template,{id:randomUUID(),storeId:tenant,name,actor:actor(request)});validateWorkflow(definition);return dependencies.workflows.put(definition,actor(request))
  },201))
...
async function assertWorkflowAccess(deps:AutomationRouteDependencies,tenant:string,used:number){const subscription=await subscriptionFor(deps,tenant);if(subscription.state==='PENDING_CONFIRMATION'||subscription.state==='PAST_DUE'||subscription.state==='SUSPENDED'||subscription.state==='CANCELLED')throw new AppError('PAYMENT_REQUIRED','Your trial or subscription is not active. Upgrade Subscription to continue.',402,{reason:'SUBSCRIPTION_REQUIRED',plan:subscription.plan});try{assertAccess(subscription,{feature:'automation_workflows',used})}catch{throw new AppError('PAYMENT_REQUIRED','Workflow limit reached. Upgrade Plan to create another workflow.',402,{reason:'UPGRADE_REQUIRED',used,limit:workflowLimit(subscription.plan),plan:subscription.plan})}}
