...
import { ActionExecutor, CalibrationLedger, CostMeter, DecisionEngine, OpenRouterClient, PostgresCalibrationStore, PostgresExecutionLedger, PostgresRecommendationRepository } from '@profitpilot/ai'
import type { ActionTool, AgentId } from '@profitpilot/ai'
import { PostgresBillingRepository, limitForPlan } from '@profitpilot/billing'
...
export function createF4Bootstrap(env: Readonly<Record<string, string | undefined>>): F4Bootstrap | null {
  ...
  const billing = new PostgresBillingRepository(f2.database)

  // PR #46: calibration survives deploys. The ledger hydrates from
  // ai_calibration_samples at first use so agent confidence caps reflect the
  // full accept/reject history, not just the current process lifetime.
  const calibration = new CalibrationLedger()
  const calibrationStore = new PostgresCalibrationStore(f2.database)
  let calibrationHydrated: Promise<void> | null = null
  const ensureCalibration = (): Promise<void> => {
    calibrationHydrated ??= calibrationStore.hydrate(calibration).catch(() => undefined)
    return calibrationHydrated
  }
  void ensureCalibration()

  const engine = new DecisionEngine(provider, costs, calibration, recommendations, { inputRateMicroDollars: numberEnv(env, 'AI_INPUT_MICRO_DOLLARS', 0), outputRateMicroDollars: numberEnv(env, 'AI_OUTPUT_MICRO_DOLLARS', 0) })
  ...
  void billing // Kept for future subscription-state checks (read-only accounts).
  ...
  const calibrationDependency = {
    record: async (storeId: StoreId, agent: AgentId, recommendationId: string, outcome: 'accepted' | 'rejected'): Promise<void> => {
      await ensureCalibration()
      calibration.record(agent, outcome)
      await calibrationStore.append(storeId, agent, recommendationId, outcome).catch(() => undefined)
    },
  }
