...
describe('Store Coach app mount (PR #48)', () => {
  ...
  it('shows the educational empty states for a store with no huddle yet', async () => {
    ...
    expect(main?.textContent ?? '').toMatch(/Welcome|getting to know your store/i)
    expect(main?.textContent ?? '').toContain('Show Me Today’s Insights')
  ...
  it('shows the plan card with factual inclusions and Upgrade Plan wording', async () => {
    ...
    expect(planCard?.textContent ?? '').toMatch(/YOUR STORE COACH INCLUDES|Right now on/)
    expect(planCard?.textContent ?? '').toContain('2 personalized priorities per day')
  ...
  it('removes chat and voice from the Store Coach home page', async () => {
    await mountApp()
    const page = document.querySelector('.coach-workspace')?.textContent ?? ''
    expect(page).not.toMatch(/Ask Coach|Voice on higher plans|Play audio|messages left today/i)
    expect(document.querySelector('.coach-chat-panel')).toBeNull()
    expect(document.querySelector('.coach-ask-section')).toBeNull()
    expect(document.querySelector('.coach-style-section')).toBeTruthy()
