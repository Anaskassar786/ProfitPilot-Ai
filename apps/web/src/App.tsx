...
import { fetchAnalytics, fetchCatalog, requestSync, ApiClientError } from './api.js'
import type { AnalyticsSnapshot, CatalogProduct, JsonValue, SectionId, WorkspaceContext } from './model.js'
...
type WorkspaceData = Readonly<{ analytics: AnalyticsSnapshot | null; catalog: readonly CatalogProduct[]; loadState: LoadState; error: string | null }>
...
export default function App() {
  ...
  const [data, setData] = useState<WorkspaceData>({ analytics: null, catalog: [], loadState: 'idle', error: null })
  ...
  const loadData = async () => {
    if (!context.storeId) {
      setData({ analytics: null, catalog: [], loadState: 'idle', error: null })
      return
    }
    setData((current) => ({ ...current, loadState: 'loading', error: null }))
    const [analyticsResult, catalogResult] = await Promise.allSettled([fetchAnalytics(context.storeId), fetchCatalog(context.storeId)])
    ...
    setData({ analytics, catalog, loadState: errors.length === 2 ? 'offline' : 'ready', error: errors[0] ? errorMessage(errors[0].reason) : null })
  }
  ...
  return (
    <div className={`app-shell ${lightMode ? 'light-mode' : ''}`}>
      ...
      <main className={`main-shell ${collapsed ? 'sidebar-is-collapsed' : ''}`}>
        ...
        <div className="page-scroll">
          ...
          <PageRouter active={activePage} context={context} data={data} onNavigate={navigate} onSync={sync} onToast={showToast} onPhaseGate={phaseGate} onEvidence={() => setEvidenceOpen(true)} lightMode={lightMode} onTheme={() => setLightMode((value) => !value)} />
        </div>
      </main>
      ...
      {evidenceOpen && <EvidenceDrawer onClose={() => setEvidenceOpen(false)} onPhaseGate={phaseGate} />}
...
function PageRouter({ active, context, data, onNavigate, onSync, onToast, onPhaseGate, onEvidence, lightMode, onTheme }: { active: SectionId; context: WorkspaceContext; data: WorkspaceData; onNavigate: (page: SectionId) => void; onSync: (module: string) => Promise<void>; onToast: (message: string, kind?: ToastKind) => void; onPhaseGate: (phase: string, capability: string) => void; onEvidence: () => void; lightMode: boolean; onTheme: () => void }) {
  ...
  if (active === 'command-center') return <CommandCenterPage onPhaseGate={onPhaseGate} />
  if (active === 'recommendations') return <RecommendationsPage onEvidence={onEvidence} onPhaseGate={onPhaseGate} />
  if (active === 'automation') return <AutomationPage onPhaseGate={onPhaseGate} />
  if (active === 'campaigns') return <CampaignsPage onPhaseGate={onPhaseGate} />
  if (active === 'copilot') return <CopilotPage onPhaseGate={onPhaseGate} />
  if (active === 'reports') return <ReportsPage onPhaseGate={onPhaseGate} />
  if (active === 'billing') return <BillingPage onPhaseGate={onPhaseGate} />
...
function CommandCenterPage({ onPhaseGate }: { onPhaseGate: (phase: string, capability: string) => void }) { return <PageLayout eyebrow="AI employee · phase gate" title="AI Command Center" description="The web shell is ready; agent execution begins only after the F4 decision engine is implemented." actions={<button className="button secondary" onClick={() => onPhaseGate('F4', 'AI decision engine execution')}><LockKeyhole size={15} /> F4 status</button>}><div className="command-health"><div><div className="section-kicker"><span className="kicker-dot purple" /> EXPLICITLY GATED</div><h2>Seven agents are designed, not pretending to run.</h2><p>Every status below is a phase boundary, not a fabricated online signal.</p></div><div className="command-health-stats"><div><strong>7</strong><span>agent contracts</span></div><div><strong>F4</strong><span>execution phase</span></div><div><strong>0</strong><span>fake decisions</span></div></div></div><div className="agent-grid">{agents.map(([name, Icon, phase]) => <div className="card agent-card" key={name}><div className="agent-card-top"><span className="agent-big-icon purple"><Icon size={19} /></span><span className="agent-status gated"><i />Gated</span><MoreHorizontal size={17} className="muted-icon" /></div><h3>{name}</h3><p>Prompt and evidence contract reserved for {phase}.</p><div className="agent-card-footer"><span><LockKeyhole size={13} /> Phase {phase}</span><span className="status-badge neutral">Not running</span></div></div>)}</div></PageLayout> }

function RecommendationsPage({ onEvidence, onPhaseGate }: { onEvidence: () => void; onPhaseGate: (phase: string, capability: string) => void }) { return <PageLayout eyebrow="AI employee · phase gate" title="Recommendations" description="The decision queue will only show immutable evidence packs from the F4 engine." actions={<><button className="button secondary" onClick={onEvidence}><Eye size={15} /> Evidence drawer</button><button className="button primary" onClick={() => onPhaseGate('F4', 'AI recommendation generation')}><LockKeyhole size={15} /> F4 gated</button></>}><div className="recommendation-summary"><div><strong>0</strong><span>recommendations displayed</span></div><div className="summary-divider" /><div className="summary-stat"><span className="confidence-dot purple" /><strong>F4</strong><small>evidence engine</small></div><div className="summary-spacer" /><span className="data-contract"><ShieldCheck size={14} /> No invented opportunities</span></div><EmptyState icon={WandSparkles} title="Decision queue is empty by design" description="When F4 is implemented, this queue will render real deterministic opportunities, evidence, confidence, and approval controls." action="Open evidence drawer" onAction={onEvidence} /></PageLayout> }

function AutomationPage({ onPhaseGate }: { onPhaseGate: (phase: string, capability: string) => void }) { return <PageLayout eyebrow="Workflow design" title="Automation" description="The visual canvas is a safe shell. Activation and worker execution are gated to F6." actions={<><button className="button secondary"><Workflow size={15} /> Templates</button><button className="button primary" onClick={() => onPhaseGate('F6', 'Workflow worker execution')}><Plus size={15} /> New workflow</button></>}><div className="automation-mode"><span className="automation-mode-icon"><ShieldCheck size={21} /></span><div><strong>Manual mode is enforced</strong><p>There is no background execution before an explicit F6 implementation.</p></div><span className="status-badge amber">F6 gated</span></div><section className="card canvas-card"><div className="canvas-toolbar"><div className="section-kicker"><Workflow size={14} /> Untitled workflow</div><span className="canvas-toolbar-note">Draft · immutable version on activate</span><button className="button secondary" onClick={() => onPhaseGate('F6', 'Workflow activation')}><Play size={14} /> Activate later</button></div><div className="dag-canvas"><DagNode title="Trigger" detail="Choose a Shopify event" icon={Radio} tone="blue" /><span className="dag-line" /><DagNode title="Condition" detail="Add a YES / NO branch" icon={GitBranch} tone="purple" /><span className="dag-line" /><DagNode title="Action" detail="Email, tag, or wait" icon={Zap} tone="amber" /></div></section></PageLayout> }

function CampaignsPage({ onPhaseGate }: { onPhaseGate: (phase: string, capability: string) => void }) { return <PageLayout eyebrow="Marketing shell" title="Campaigns" description="Campaign composition is reserved for the F6 marketing center and real merchant email configuration." actions={<><button className="button secondary"><ListFilter size={15} /> Template gallery</button><button className="button primary" onClick={() => onPhaseGate('F6', 'Campaign execution')}><Plus size={15} /> New campaign</button></>}><div className="campaign-hero"><div><div className="section-kicker"><span className="kicker-dot purple" /> Compliance first</div><h2>No message will be sent from the web shell.</h2><p>Suppression ledgers, unsubscribe links, HMAC tracking, and attribution must be wired before delivery.</p></div><div className="campaign-hero-art"><Mail size={28} /><span>F6</span></div></div><EmptyState icon={Mail} title="No campaigns yet" description="Configure merchant email and enable the F6 campaign center before drafting customer messages." action="View phase boundary" onAction={() => onPhaseGate('F6', 'Campaign creation')} /></PageLayout> }
...
function ExportsPage({ context }: { context: WorkspaceContext }) {
  const exportTypes: ReadonlyArray<{ title: string; icon: LucideIcon; format: string }> = [
    { title: 'Orders export', icon: ShoppingBag, format: 'CSV · XLSX' },
    { title: 'Catalog export', icon: Package, format: 'CSV · XLSX' },
    { title: 'Audit log export', icon: ShieldCheck, format: 'CSV' },
    { title: 'Revenue report', icon: FileBarChart, format: 'PDF' },
  ]
  return <PageLayout eyebrow="Data portability" title="Exports" description="Export surfaces are ready to receive real sync rows from the data plane." actions={<button className="button secondary"><Clock3 size={15} /> Export history</button>}>
    <div className="export-intro"><div><div className="section-kicker"><span className="kicker-dot blue" /> No synthetic rows</div><h2>{context.storeId ? 'Choose a real dataset to export.' : 'Connect a store before exporting.'}</h2><p>CSV, XLSX, and PDF writers will be added in their respective phases.</p></div><span className="export-limit"><strong>50,000</strong><small>row ceiling</small></span></div>
    <div className="export-grid">{exportTypes.map(({ title, icon: Icon, format }) => <div className="card export-card" key={title}><span className="export-icon blue"><Icon size={20} /></span><h3>{title}</h3><p>Writer activates when the corresponding data surface is ready.</p><div className="export-card-bottom"><span>{format}</span><button className="button secondary" disabled>Not enabled</button></div></div>)}</div>
  </PageLayout>
}

function SupportPage() { return <PageLayout eyebrow="Operator inbox" title="Support tickets" description="A support inbox shell for auditable merchant ↔ operator threads."><div className="support-hero"><span className="support-hero-icon"><LifeBuoy size={22} /></span><div><div className="section-kicker">SUPPORT CHANNEL</div><h2>No open tickets.</h2><p>When an operator inbox is connected, status and priority will appear here.</p></div><span className="support-sla"><strong>—</strong><small>SLA appears with billing plan</small></span></div><EmptyState icon={Inbox} title="Your support inbox is clear" description="Create a ticket when there is a real question for the ProfitPilot team." action="New ticket" onAction={() => undefined} /></PageLayout> }

function BillingPage({ onPhaseGate }: { onPhaseGate: (phase: string, capability: string) => void }) { return <PageLayout eyebrow="Entitlements" title="Billing" description="Billing state is secure on the backend; full plan actions are intentionally gated to F5." actions={<button className="button primary" onClick={() => onPhaseGate('F5', 'Shopify billing actions')}><LockKeyhole size={15} /> F5 gated</button>}><div className="billing-current"><div className="billing-plan"><span className="plan-icon"><WalletCards size={19} /></span><div><div className="section-kicker">CURRENT PLAN</div><h2>Not connected <span>· no entitlement data</span></h2><p>Plans and usage will render from the billing state machine in F5.</p></div><span className="status-badge neutral">Unavailable</span></div></div><div className="billing-grid"><section className="card usage-panel"><CardHeading kicker="Usage meters" dot="blue" title="Awaiting entitlements" /><Quota label="AI recommendations" value="—" percent={0} /><Quota label="Jarvis messages" value="—" percent={0} /><Quota label="Email sends" value="—" percent={0} /><button className="text-button full" onClick={() => onPhaseGate('F5', 'Usage entitlement UI')}>View F5 boundary <ArrowUpRight size={14} /></button></section><section className="card roi-panel"><CardHeading kicker="Return on AI" dot="gold" title="No attribution yet" /><div className="gated-panel compact"><LockKeyhole size={20} /><strong>F4 + F5 required</strong><p>ROI needs verified attribution and billing cost data.</p></div></section></div><div className="plan-cards">{['Start', 'Growth', 'Commander'].map((name) => <div className="plan-card" key={name}><h3>{name}</h3><div className="plan-price"><strong>—</strong><span>pricing in F5</span></div><p>No plan copy is shown before the live billing contract.</p><button className="button ghost" onClick={() => onPhaseGate('F5', 'Plan selection')}>View phase boundary</button></div>)}</div></PageLayout> }

function SettingsPage({ context, lightMode, onTheme, onToast }: { context: WorkspaceContext; lightMode: boolean; onTheme: () => void; onToast: (message: string, kind?: ToastKind) => void }) { return <PageLayout eyebrow="Workspace controls" title="Settings" description="Store context and accessibility preferences for the current web shell."><div className="settings-layout"><aside className="settings-nav card"><button className="settings-nav-item active"><Settings size={15} /> General</button><button className="settings-nav-item"><Bell size={15} /> Notifications</button><button className="settings-nav-item"><Bot size={15} /> Jarvis preferences</button><button className="settings-nav-item"><Users size={15} /> Team members</button><button className="settings-nav-item"><ShieldCheck size={15} /> Security & audit</button><button className="settings-nav-item danger"><Trash2 size={15} /> Danger zone</button></aside><div className="settings-panels"><SettingsPanel title="Store context" description="The UI reads this context from the embedded Shopify URL and F2 API."><SettingRow label="Shopify store" description="No store name is fabricated"><span className="setting-readonly">{context.shop ?? 'Not provided'}</span></SettingRow><SettingRow label="Tenant id" description="Used for tenant-scoped F2 requests"><span className="setting-readonly mono">{context.storeId ?? 'Not provided'}</span></SettingRow></SettingsPanel><SettingsPanel title="Appearance" description="Dark mode is the default ProfitPilot surface."><SettingRow label="Theme" description="Optional light mode for daytime work"><div className="theme-choice"><button className={!lightMode ? 'selected' : ''} onClick={() => lightMode && onTheme()}><Moon size={15} /> Dark</button><button className={lightMode ? 'selected' : ''} onClick={() => !lightMode && onTheme()}><Sun size={15} /> Light</button></div></SettingRow><SettingRow label="Reduced motion" description="Respect the operating system preference"><Toggle on={false} /></SettingRow></SettingsPanel><SettingsPanel title="Data trust" description="These indicators describe actual boundary state."><SettingRow label="F2 analytics" description="Read from /analytics when a storeId is present"><span className="status-badge green"><CheckCircle2 size={12} /> API-backed</span></SettingRow><SettingRow label="AI recommendations" description="No AI number is rendered before F4"><span className="status-badge purple"><LockKeyhole size={12} /> F4 gated</span></SettingRow></SettingsPanel><div className="settings-save"><span><ShieldCheck size={15} /> No unsaved fake state</span><button className="button primary" onClick={() => onToast('Preferences are saved locally for this shell.', 'success')}>Save preferences</button></div></div></div></PageLayout> }
...
function EvidenceDrawer({ onClose, onPhaseGate }: { onClose: () => void; onPhaseGate: (phase: string, capability: string) => void }) { return <><button className="drawer-backdrop" onClick={onClose} aria-label="Close evidence drawer" /><aside className="evidence-drawer"><div className="drawer-header"><div><span className="drawer-kicker"><Database size={13} /> EVIDENCE PACK</span><h2>No evidence yet</h2></div><button className="icon-button" onClick={onClose}><X size={18} /></button></div><div className="drawer-scroll"><div className="gated-panel"><LockKeyhole size={22} /><strong>F4 evidence engine required</strong><p>The drawer exists now, but no customer or revenue evidence is fabricated in F3.</p><span className="phase-tag">Phase F4</span></div><div className="drawer-section"><div className="drawer-section-title"><ShieldCheck size={15} /> Safety contract</div><div className="safety-list"><span><Check size={14} /> Immutable evidence hash</span><span><Check size={14} /> PII minimization</span><span><Check size={14} /> CAS approval state</span></div></div></div><div className="drawer-footer"><button className="button secondary" onClick={onClose}>Close</button><button className="button primary" onClick={() => onPhaseGate('F4', 'Evidence pack generation')}>View F4 gate</button></div></aside></> }
