...
const navGroups: ReadonlyArray<{ label: string; items: ReadonlyArray<NavItem> }> = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'admin-ops', label: 'Admin Ops', icon: ShieldCheck, tag: 'F9' },
    ],
  },
]
...
function PageRouter({ active, context, data, onNavigate, onSync, onSyncAll, syncProgress, syncAllRunning, onDecide, onRefresh, onToast, onPhaseGate, onEvidence, lightMode, onTheme }: { active: SectionId; context: WorkspaceContext; data: WorkspaceData; onNavigate: (page: SectionId) => void; onSync: (module: string) => Promise<void>; onSyncAll: () => Promise<void>; syncProgress: readonly SyncModuleProgress[]; syncAllRunning: boolean; onDecide: (id: string, decision: 'approve' | 'reject', expectedVersion: number) => Promise<void>; onRefresh: () => void; onToast: (message: string, kind?: ToastKind) => void; onPhaseGate: (phase: string, capability: string) => void; onEvidence: () => void; lightMode: boolean; onTheme: () => void }) {
  ...
  if (active === 'copilot') return <PageLayout eyebrow="Advanced query · F8" title="Copilot" description="A closed ten-intent grammar answers from tenant-scoped evidence packs."><CopilotWorkspace context={context} /></PageLayout>
  if (active === 'reports') return <PageLayout eyebrow="Reporting vault · F8" title="Reports" description="Closed-period PDF reports, deterministic forecast methods, and honest delivery status."><ReportsWorkspace context={context} /></PageLayout>
  if (active === 'admin-ops') return <PageLayout eyebrow="Launch hardening · F9" title="Admin Ops" description="Final controls for maintenance, merchant flags, queues, and operational recovery."><AdminOpsWorkspace context={context} /></PageLayout>
