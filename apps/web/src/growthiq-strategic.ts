...
/** Ordinary least squares over y = f(index). */
function fitLine(values: readonly number[]): Readonly<{ slope: number; intercept: number; r2: number; residualStd: number }> {
  ...
  for (let index = 0; index < n; index += 1) {
    sxx += (index - meanX) ** 2
    sxy += (index - meanX) * (values[index]! - meanY)
  }
  ...
  let sst = 0
  for (let index = 0; index < n; index += 1) {
    const residual = values[index]! - (slope * index + intercept)
    sse += residual ** 2
    sst += (values[index]! - meanY) ** 2
  }
...
function addDays(day: string, count: number): string {
  ...
  return new Date(date.getTime() + count * 86_400_000).toISOString().slice(0, 10)
}
...
export function projectTrajectory(series: readonly DayPoint[]): TrajectoryProjection | null {
  const usable = series.filter((point) => Number.isFinite(point.value))
  if (usable.length < 2) return null
  const historical = usable.slice(-30)
  const { slope, intercept, r2, residualStd } = fitLine(historical.map((point) => point.value))
  ...
  const currentMonthlyRunRate = (historical.reduce((sum, point) => sum + point.value, 0) / historical.length) * 30
  const projectedMonthlyRevenue = projected.reduce((sum, point) => sum + point.value, 0)
  const growthRatePct = currentMonthlyRunRate > 0 ? (projectedMonthlyRevenue / currentMonthlyRunRate - 1) * 100 : null
...
export function momentumScore(growthRatePct: number | null): number | null {
  if (growthRatePct === null) return null
...
export function strategicPosition(input: Readonly<{ revenuePercentile: number | null; growthRatePct: number | null }>): StrategicPosition {
  const x = input.revenuePercentile === null ? null : Math.min(100, Math.max(0, input.revenuePercentile))
...
export function trailingWindows(series: readonly DayPoint[], days: number): Readonly<{ current: number; prior: number | null }> {
  const current = series.slice(-days).reduce((sum, point) => sum + point.value, 0)
  const priorSlice = series.slice(-days * 2, -days)
  const prior = priorSlice.length > 0 ? priorSlice.reduce((sum, point) => sum + point.value, 0) : null
...
const money0 = (value: number, currency: string): string =>
  new Intl.NumberFormat('en-US', { style: 'currency', currency, maximumFractionDigits: 0 }).format(Math.round(value))
...
export function impactPreviews(input: Readonly<{
  ...
  if (aov && aov.yourValue !== null && aov.industryMedian !== null && input.orders30 > 0) {
  ...
  if (repeat && repeat.yourValue !== null && repeat.industryMedian !== null) {
...
export function growthMilestones(input: Readonly<{
  ...
  const milestones: GrowthMilestone[] = defs.map((def) => {
    const complete = def.current >= def.target
    ...
    return { key: def.key, title: def.title, target: def.target, current: def.current, metric: def.metric, status, progressPct: Math.min(100, Math.round((def.current / def.target) * 100)) }
  })
  ...
  if (active && input.daysSynced > 0) {
    const pace = active.current / input.daysSynced
    if (pace > 0 && active.current < active.target) {
  ...
  const stage: GrowthMilestonesResult['stage'] =
    input.customers >= 50 || input.syncedOrders >= 100 ? 'Expansion'
      : input.syncedOrders >= 10 || input.customers >= 10 ? 'Early growth'
        : input.syncedOrders >= 1 ? 'Launch'
          : 'Foundation'
...
export function weeklyDigest(input: Readonly<{
  ...
  if (input.revenueSeries.length < 7) return null
  ...
  if (revenueWowPct !== null && revenueWowPct <= -8) {
  ...
  } else if (topShare !== null && topShare >= 55) {
  ...
  } else if (input.repeatRatePct !== null && input.repeatMedianPct !== null && input.repeatRatePct < input.repeatMedianPct) {
  ...
  } else if (revenueWowPct !== null && revenueWowPct >= 10) {
