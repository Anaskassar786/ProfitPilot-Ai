...
describe('GrowthIqTrajectorySection', () => {
  ...
  it('degrades to honest education with a single synced day', () => {
    const html = renderToStaticMarkup(<GrowthIqTrajectorySection projection={null} currency="USD" daysSynced={1} onNavigateReports={() => undefined} />)
...
describe('GrowthIqPositionSection', () => {
  it('plots the quadrant from measured momentum and percentile', () => {
    ...
    const html = renderToStaticMarkup(<GrowthIqPositionSection position={position} nextMilestone="25 customers" onNavigateBenchmarks={() => undefined} />)
  ...
  it('explains precisely why the position cannot plot yet', () => {
    ...
    const html = renderToStaticMarkup(<GrowthIqPositionSection position={position} nextMilestone={null} onNavigateBenchmarks={() => undefined} />)
...
describe('GrowthIqImpactSection', () => {
  it('prints computed impacts and honest nulls side by side', () => {
    ...
    const html = renderToStaticMarkup(<GrowthIqImpactSection previews={previews} onNavigate={() => undefined} />)
...
describe('GrowthIqMilestonesSection', () => {
  it('shows completes, the active milestone with progress, and locked horizon', () => {
    ...
    const html = renderToStaticMarkup(<GrowthIqMilestonesSection result={result} onNavigateRoadmaps={() => undefined} />)
...
describe('GrowthIqDigestSection', () => {
  ...
  it('renders the board-style snapshot from real weekly data', () => {
    ...
    const html = renderToStaticMarkup(<GrowthIqDigestSection digest={digest} daysSynced={14} currency="USD" plan="growth" canReadReports onNavigateReports={() => undefined} onUpgrade={() => undefined} />)
  ...
  it('gates the full report with an Upgrade Plan CTA — never a plan name', () => {
    const digest = weeklyDigest(digestInput)!
    const html = renderToStaticMarkup(<GrowthIqDigestSection digest={digest} daysSynced={14} currency="USD" plan="trial" canReadReports={false} onNavigateReports={() => undefined} onUpgrade={() => undefined} />)
  ...
  it('reports the real sync count when the digest has not unlocked', () => {
    const html = renderToStaticMarkup(<GrowthIqDigestSection digest={null} daysSynced={3} currency="USD" plan="trial" canReadReports={false} onNavigateReports={() => undefined} onUpgrade={() => undefined} />)
...
describe('GrowthIqActionsPanel', () => {
  it('renders all four executive actions', () => {
    const html = renderToStaticMarkup(<GrowthIqActionsPanel onNavigate={() => undefined} />)
