...
describe('AI Command light theme stays scoped to light mode', () => {
  ...
  it('scopes every new light override to .app-shell.light-mode (dark theme untouched)', () => {
    ...
    expect(css).toContain('.app-shell.light-mode .aic-shell { background: rgb(248, 250, 252); }')
    expect(css).toContain('.app-shell.light-mode .aic-title h2 { color: rgb(15, 23, 42); }')
    expect(css).toContain('.app-shell.light-mode .aic-composer textarea { background: rgb(255, 255, 255); border-color: rgb(226, 232, 240); color: rgb(15, 23, 42); }')
    expect(css).toContain('.app-shell.light-mode .aic-bubble.mine .aic-bubble-body { background: rgb(243, 232, 255);')
    expect(css).toContain('.app-shell.light-mode .aic-planbar { background: linear-gradient(90deg, rgb(254, 243, 199), rgb(253, 230, 138));')
  })
})
