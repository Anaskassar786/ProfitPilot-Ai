...
export const QA_AREAS: readonly QaArea[] = [
  ...
  },
  {
    ...
    outcome: 'FIXED',
    checks: [
      { name: 'Dashboard + section lists load', status: 'FIXED', note: 'All /ai-executive/* GETs return 200 with real rows. FIXED: dashboard used to fail intermittently (4/30 runs) with transient 500s ("portal does not exist" / non-numeric row) under concurrent pooled reads — now serialized reads + protocol retry: 0/50 stress runs.' },
  ...
  },
  {
    ...
    checks: [
      ...
      { name: 'Overview never hangs', status: 'FIXED', note: 'BEFORE: auto-discovery ran in-band and a stalled narrator could hang /patternai/overview 15s+ (client aborted). FIXED: pipeline bounded to 8s — overview always renders; verified 10/10 runs at 15-44ms on both stores.' },
...
export const QA_BUGS: readonly QaBug[] = [
  ...
  },
  {
    id: 'BUG-09',
    severity: 'P1',
    area: 'GrowthIQ dashboard (intermittent)',
    symptom: 'GET /ai-executive/dashboard failed ~4 times in 30 runs with 500 "Internal server error" — either `portal "" does not exist` or "Analytics database returned a non-numeric value".',
    rootCause: 'The dashboard read the four analytics tables as four concurrent extended-protocol queries over pooled connections. Under a connection-multiplexing proxy the streams can interleave and glitch a portal, and a glitched response can hand back a row with an undefined column.',
    fix: 'PostgresAnalyticsRepository.read is now sequential (deterministic, halves peak pool usage); PostgresDatabase.query retries once on transient protocol errors (portal / connection terminated); the read re-fetches once if a glitched row yields a non-numeric value. Stress test: 4/30 failures → 0/50.',
    status: 'FIXED',
  },
  {
    id: 'BUG-10',
    severity: 'P1',
    area: 'PatternAI overview',
    symptom: 'GET /patternai/overview could hang for 15s+ (client aborted, no response) when auto-discovery was due — a stalled AI narrator blocked the whole page response in-band.',
    rootCause: 'runDiscoveryPipeline (including per-discovery narrator calls) awaited inline; its .catch() could not help because the call was slow, not rejected.',
    fix: 'Auto-discovery is bounded by an 8-second race — the page always renders even when the provider stalls; the pipeline continues in the background and its writes are idempotent.',
    status: 'FIXED',
  },
  {
    id: 'BUG-08',
...
export const QA_FINAL_VERDICT = {
  ...
  nextSteps: [
    ...
    'Optional hardening already included in this PR: transient DB-protocol retry + bounded PatternAI auto-discovery.',
