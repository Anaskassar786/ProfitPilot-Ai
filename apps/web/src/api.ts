...
export function askCopilot(storeId: string, query: string, page: string, threadId?: string, fetcher: Fetcher = fetch): Promise<CopilotAnswer> { return requestJson<CopilotAnswer>('/copilot/query', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ storeId, query, page, ...(threadId ? { threadId } : {}) }) }, fetcher) }
