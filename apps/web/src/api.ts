...
export type MerchantEmailConfigView = Readonly<{ shopId: string; merchantEmail: string; fromName: string; verified: boolean; verificationSentAt: number | null; verifiedAt: number | null }>
export type MerchantEmailSaveResult = Readonly<{ config: Readonly<Record<string, unknown>>; verificationToken: string; verificationRequired?: boolean; emailSent?: boolean }>
export function fetchMerchantEmail(shopId: string, fetcher: Fetcher = fetch): Promise<MerchantEmailConfigView | null> { return requestJson<MerchantEmailConfigView | null>(`/settings/merchant-email?shopId=${encodeURIComponent(shopId)}`, {}, fetcher) }
export function saveMerchantEmail(shopId: string, email: string, fromName: string, fetcher: Fetcher = fetch): Promise<MerchantEmailSaveResult> { return requestJson(`/settings/merchant-email`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ shopId, email, fromName }) }, fetcher) }
...
export function fetchWorkspaceSettings(storeId: string, fetcher: Fetcher = fetch): Promise<Readonly<Record<string, unknown>>> { return requestJson(`/settings/workspace?storeId=${encodeURIComponent(storeId)}`, {}, fetcher) }
export function saveWorkspaceSettings(storeId: string, settings: Readonly<Record<string, unknown>>, fetcher: Fetcher = fetch): Promise<Readonly<Record<string, unknown>>> { return requestJson('/settings/workspace', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ storeId, ...settings }) }, fetcher) }
...
export function fetchCoachAvailableAchievements(storeId: string, fetcher: Fetcher = fetch): Promise<Readonly<{ earnedIds: readonly string[]; catalog: readonly import('./store-coach-model.js').CoachBadgeCatalogEntry[]; categories: readonly import('./store-coach-model.js').CoachBadgeCategoryStat[]; visible: number }>> { return requestJson(coachPath('/store-coach/achievements/available', storeId), {}, fetcher) }
