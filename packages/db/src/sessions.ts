...
import { PostgresDatabase, withStoreContext } from './database.js'
...
export interface SessionRepository {
  ...
  getForStore?(storeId: StoreId, id: string): Promise<SessionRecord | null>
...
export class InMemorySessionRepository implements SessionRepository {
  ...
  public async getForStore(storeId: StoreId, id: string): Promise<SessionRecord | null> {
    const session = await this.get(id)
    return session?.storeId === storeId ? session : null
  }
...
export class PostgresSessionRepository implements SessionRepository {
  ...
  public async get(id: string): Promise<SessionRecord | null> {
    return this.read(this.executor, id)
  }

  public async getForStore(storeId: StoreId, id: string): Promise<SessionRecord | null> {
    if (this.executor instanceof PostgresDatabase) {
      return this.executor.withTransaction(async (client) => withStoreContext(client, storeId, () => this.read(client, id)))
    }
    const result = await this.executor.query<SessionRow>('SELECT * FROM auth_sessions WHERE id = $1 AND store_id = $2 LIMIT 1', [id, storeId])
  ...
  private async read(executor: SqlExecutor, id: string): Promise<SessionRecord | null> {
    const result = await executor.query<SessionRow>('SELECT * FROM auth_sessions WHERE id = $1 LIMIT 1', [id])
    const row = result.rows[0]
    return row ? fromRow(row) : null
  }
}
