...
import { databaseConfigFromEnv, F0_MIGRATIONS, pendingMigrations, withStoreContext } from './index.js'
...
describe('F0 migrations', () => {
  ...
  it('returns unapplied migrations in order', () => expect(pendingMigrations(['0001']).map((migration) => migration.id)).toEqual(['0002']))
  it('returns no work when current', () => expect(pendingMigrations(['0001', '0002'])).toEqual([]))
})
