...
describe('billing repositories', () => {
  it('stores one account per shop', async () => { const repo = new InMemoryBillingRepository(); await repo.put(record); await repo.put({ ...record, state: 'PAST_DUE' }); expect((await repo.get('s'))?.state).toBe('ACTIVE_MONTHLY') })
