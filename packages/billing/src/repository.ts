...
export class PostgresBillingRepository implements BillingRepository {
  ...
  public async get(shopId: string): Promise<BillingRecord | null> { const result = await this.executor.query<BillingRow>('SELECT state, plan, interval, current_period_end, version, price_locked_at, grandfathered, NULL AS charge_id FROM billing_subscriptions WHERE shop_id = $1 LIMIT 1', [shopId]); const row = result.rows[0]; return row ? mapRow(shopId, row) : null }
  public async put(record: BillingRecord): Promise<void> { await this.executor.query(`INSERT INTO billing_subscriptions (shop_id, state, plan, interval, current_period_end, version, price_locked_at, grandfathered) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) ON CONFLICT (shop_id) DO UPDATE SET state = EXCLUDED.state, plan = EXCLUDED.plan, interval = EXCLUDED.interval, current_period_end = EXCLUDED.current_period_end, version = EXCLUDED.version, price_locked_at = EXCLUDED.price_locked_at, grandfathered = EXCLUDED.grandfathered, updated_at = now()`, [record.storeId, record.state, record.plan, record.interval, record.currentPeriodEnd, record.version, record.priceLockedAt ?? null, record.grandfathered ?? false]) }
  public async transition(shopId: string, expectedVersion: number, state: BillingState, now: number): Promise<BillingRecord> { const result = await this.executor.query<BillingRow>(`UPDATE billing_subscriptions SET state = $3, version = version + 1, updated_at = to_timestamp($4 / 1000.0) WHERE shop_id = $1 AND version = $2 RETURNING state, plan, interval, current_period_end, version, price_locked_at, grandfathered, NULL AS charge_id`, [shopId, expectedVersion, state, now]); const row = result.rows[0]; if (!row) throw new AppError('CONFLICT', 'Billing subscription changed; reload before retrying', 409, { shopId, expectedVersion }); return mapRow(shopId, row) }
}
