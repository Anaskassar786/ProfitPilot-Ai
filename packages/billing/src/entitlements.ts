...
export class UpgradeRequiredError extends AppError {
  public constructor(feature: EntitlementKey | `agent:${string}`, plan: PlanTier, requiredPlan?: PlanTier) { super('FORBIDDEN', `Upgrade required for ${feature}`, 403, { feature, plan, reason: 'UPGRADE_REQUIRED', ...(requiredPlan ? { requiredPlan } : {}) }); this.name = 'UpgradeRequiredError' }
}
