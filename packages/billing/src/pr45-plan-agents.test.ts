...
describe('PR45 plan → agent matrix', () => {
  ...
  it('growth adds Pricing and Campaign (5 total)', () => {
    expect(agentsForPlan('growth')).toHaveLength(5)
    expect(agentsForPlan('growth')).toEqual(expect.arrayContaining(['PRICING_AGENT', 'CAMPAIGN_AGENT']))
    expect(agentsForPlan('growth')).not.toContain('PRODUCT_AGENT')
  })
  it('commander unlocks all seven', () => {
    expect(agentsForPlan('commander')).toHaveLength(7)
  ...
  it('plan definitions expose the named agent lists', () => {
    ...
    expect(PLAN_DEFINITIONS.GROWTH.agents).toHaveLength(5)
    expect(PLAN_DEFINITIONS.COMMANDER.agents).toHaveLength(7)
    expect(PLAN_DEFINITIONS.GROWTH.limits.active_agents).toBe(5)
  })
})
...
describe('PR45 agent gating', () => {
  it('reports the cheapest plan that unlocks each agent', () => {
    ...
    expect(requiredPlanForAgent('CAMPAIGN_AGENT')).toBe('growth')
  ...
  it('allows unlocked agents and blocks locked ones', () => {
    ...
    expect(agentAccess('growth', 'CAMPAIGN_AGENT').allowed).toBe(true)
