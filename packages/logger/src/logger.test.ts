...
import { Logger, createMemorySink, loggerFromEnv, mergeFields, redactFields } from './index.js'
...
describe('structured logger', () => {
  ...
  it('normalizes configured production log levels', () => {
    expect(loggerFromEnv({ LOG_LEVEL: 'debug' })).toBeInstanceOf(Logger)
    expect(loggerFromEnv({ LOG_LEVEL: 'invalid' })).toBeInstanceOf(Logger)
    loggerFromEnv({ LOG_LEVEL: 'error' }).error('configured')
    loggerFromEnv({ LOG_LEVEL: 'warn' }).warn('configured warning')
  })
})
