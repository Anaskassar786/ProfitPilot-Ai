...
export class PostgresWebhookProcessingStore implements WebhookProcessingStore {
  ...
  public async begin(event: WebhookEvent, now = Date.now()): Promise<boolean> {
    ...
    const result = await this.executor.query<ReceiptRow>(
    ...
    await this.writeAudit(event.storeId, event.webhookId, 'claimed', result.rows[0]?.attempts === 1 ? 'first attempt' : 'retry attempt', now)
    return true
  }

  public async markProcessed(storeId: StoreId, webhookId: string, now = Date.now()): Promise<void> {
    const result = await this.executor.query(`UPDATE webhook_receipts SET status = 'PROCESSED', processed_at = to_timestamp($3 / 1000.0), next_attempt_at = NULL WHERE store_id = $1 AND webhook_id = $2`, [storeId, webhookId, now])
    if (result.rowCount === 0) throw new Error('Cannot process an unknown webhook receipt')
    await this.writeAudit(storeId, webhookId, 'processed', 'handler completed', now)
  }
  ...
  public async markFailed(storeId: StoreId, webhookId: string, reason: string, now = Date.now()): Promise<WebhookStatus> {
    ...
    await this.executor.query(`UPDATE webhook_receipts SET status = $3, last_error = $4, next_attempt_at = CASE WHEN $5 IS NULL THEN NULL ELSE to_timestamp($5 / 1000.0) END, failed_at = CASE WHEN $5 IS NULL THEN to_timestamp($6 / 1000.0) ELSE NULL END WHERE store_id = $1 AND webhook_id = $2`, [storeId, webhookId, status, reason, nextAttemptAt, now])
    await this.writeAudit(storeId, webhookId, terminal ? 'failed' : 'retry_scheduled', reason, now)
    return status
  }

  public async get(storeId: StoreId, webhookId: string): Promise<WebhookReceipt | null> {
    const result = await this.executor.query<ReceiptRow>('SELECT store_id, webhook_id, topic, payload_hash, status, attempts, next_attempt_at, last_error, received_at, processed_at, failed_at FROM webhook_receipts WHERE store_id = $1 AND webhook_id = $2 LIMIT 1', [storeId, webhookId])
  ...
  private async writeAudit(storeId: StoreId, webhookId: string, event: WebhookAuditEvent['event'], detail: string, at: number): Promise<void> {
    await this.executor.query('INSERT INTO webhook_audit_events (store_id, webhook_id, event, detail, at) VALUES ($1, $2, $3, $4, to_timestamp($5 / 1000.0))', [storeId, webhookId, event, detail, at])
  }
}
