...
export type EmailAttachment = Readonly<{ filename: string; content: Buffer; contentType: string }>
export type EmailMessage = Readonly<{ to: string; from: string; fromName: string; subject: string; html: string; headers?: Readonly<Record<string, string>>; attachments?: readonly EmailAttachment[] }>
...
export class SmtpMailer implements EmailTransport {
  ...
  public async send(message: EmailMessage): Promise<Readonly<{ messageId: string }>> { const info = await this.transport.sendMail({ to: message.to, from: `${message.fromName} <${message.from}>`, subject: message.subject, html: message.html, headers: message.headers, attachments: message.attachments?.map((attachment) => ({ filename: attachment.filename, content: attachment.content, contentType: attachment.contentType })) }); return { messageId: info.messageId }
  }
}
...
export class MerchantEmailVerifier {
  ...
  /** Hydrates a durable, server-read configuration before campaign delivery. */
  public hydrate(config: MerchantEmailConfig): MerchantEmailConfig { this.configs.set(config.shopId, { ...config }); return config }
